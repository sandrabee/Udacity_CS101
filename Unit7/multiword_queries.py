        ...
        for i in query:
             if lookup(index, i):
                for j in range(len(index[i])):
                     single.append(index[i][j][0])
        return single
    else:
        for i in query:
            if lookup(index, i):
                for j in range(len(index[i])):
                    if index[i][j][0] in temp:
                        temp[index[i][j][0]].append(index[i][j][1])
                    else:
                        temp[index[i][j][0]]=[index[i][j][1]]
            for i in temp:
                l = temp[i]
                l_short = len(l)-1
                for no, elem in enumerate(l[:l_short]):
                    this_elem = elem
                    next_elem = l[(no + 1) % len(l)]
                    if next_elem-this_elem == 1:
                        multi.append(i)
            for i in multi:
                 if multi.count(i) == l_short:
                     if i not in final:
                         final.append(i)
    return final
...
def add_to_index(index, keyword, i, url):
        if keyword in index:
